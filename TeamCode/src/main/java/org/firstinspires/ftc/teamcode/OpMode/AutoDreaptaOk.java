...
@Autonomous(name="Auto Dreapta Ok👉👌")
public class AutoDreaptaOk extends LinearOpMode {
    ...
    Pose2d[] conePickup = {new Pose2d(50,-23,-1.5*Math.PI),
            new Pose2d(49,-23,-1.5*Math.PI),
            new Pose2d(47,-23.5,-1.5*Math.PI),
            new Pose2d(46,-23.5,-1.5*Math.PI),
            new Pose2d(45,-24.8,-1.5*Math.PI)
    };

    Pose2d[] coneRelease = {new Pose2d(52.5, 16, -5.851),
            new Pose2d(52, 16, -5.851),
            new Pose2d(51.5, 16, -5.851),
            new Pose2d(50, 17, -5.851),
            new Pose2d(49, 18, -5.851),
            new Pose2d(49, 19, -5.851)
    };
        ...
        while(opModeIsActive() && !isStopRequested()) {
            ...
            if(!driveTrain.isBusy()){
                if(index<=conesFromStack){
                    if(robotModules.outtake.state == Outtake.State.HIGH && nanoClock.seconds() - robotModules.outtake.lift.timeOfLastStateChange >= upWaitTime[index]) robotModules.outtake.setState(Outtake.State.GOING_DOWN);
